...
class RootHandler implements HttpHandler {
	int port = 8081;
	String response2 = "<html>\n" +  
			"<head><title>Practica A.S.D.</title>\n" + 
			"</head>\n"+
			"<p><h1>Arquitectura y Servicios Distribuidos</h1></p>\n" +
			"<p><h2>Autor: Saltos Mero Johnny Samuel</h2></p>\n" +
			"<p>Server start success if you see this message\"  \" Port: \"  8081   </p>\n" +
			"<a href=\"/echoGet\">Metodo GET</a>\n"+
			"<a href=\"/echoPost\">Metodo POST</a>\n"+
			"<a href=\"/echoHeader\">contenido HEADER</a>"+
			"</body>\n" + 
			"</html>\n";
...
class EchoGetHandler implements HttpHandler {
         ...
         public void handle(HttpExchange he) throws IOException {
                 ...
                 File f = new File("calc.html");
                 FileReader reader = new FileReader(f);
                 BufferedReader buff = new BufferedReader(reader);
                 temp=buff.readLine();
                 while (temp!=null) {
                	// System.out.println("Leido: "+temp);
                	 response = response + temp + "\n";
                	 temp = buff.readLine();                			 
                 }
                 System.out.println("Leido:"+response.length()+ "bytes");
                 reader.close();
                 he.sendResponseHeaders(200, response.length());
                 OutputStream os = he.getResponseBody();
                 os.write(response.toString().getBytes());
